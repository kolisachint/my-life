...
Read the `career-docs` skill first. It has the fifteen consumers, the
...
3. Apply the change to **`data/career-facts.md` first**, then to every consumer
   it names. There are fifteen and the file lists them — do not update three and
   call it done.
...
6. **`bin/docx --check`** — re-renders all six Word copies from the SAME HTML,
...
**Before finishing, state plainly:** which of the fifteen changed, which you left
