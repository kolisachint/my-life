// Sachin_Koli_Resume_ATS.docx — deliberately plain. One column, no tables, no text boxes,
  ...
  return new Document({
    ...
    sections: [{
      ...
      children: [
        ...
        line('Machine Learning and AI: Large language models (LLM), LLM agents, agentic workflows, tool calling, function calling, Model Context Protocol (MCP), vector embeddings, vector search, semantic search, approximate nearest neighbour (ANN), HNSW, hybrid retrieval, BM25, retrieval augmented generation (RAG), ONNX Runtime, sentence transformers, automatic speech recognition (ASR), Rust, TypeScript', { after: 60 }),
        ...
        line('hoocode - github.com/kolisachint/hoocode (TypeScript)', { bold: true, after: 40 }),
        bullet('Deterministic terminal coding agent published to npm as @kolisachint/hoocode-agent. Four packages: CLI, agent runtime with tool calling and state management, unified LLM API across 25+ providers, and a terminal UI library with differential rendering.'),
        bullet('Permission gating on every file edit and shell command, four scoped agent modes, Model Context Protocol (MCP) server support and subagent delegation.'),
        line('embeddingsearchtools - github.com/kolisachint/embeddingsearchtools (Rust)', { bold: true, after: 40 }),
        bullet('Embedding search engine. Sentence-transformer inference (all-MiniLM-L6-v2) via ONNX Runtime with int8 quantised weights bundled into the binary.'),
        bullet('Exact and HNSW approximate nearest neighbour vector indexes implemented from scratch; BM25 lexical index fused for hybrid retrieval; memory-mapped persistence exposed through a library API, CLI and long-lived daemon.'),
        line('voicetools - github.com/kolisachint/voicetools (Rust)', { bold: true, after: 40 }),
        bullet('Offline automatic speech recognition for the terminal. Microphone capture, voice activity detection and resampling into Parakeet-TDT on ONNX Runtime across 25 languages, with a Whisper fallback backend.'),
        line('webtools - github.com/kolisachint/webtools (Rust)', { bold: true, after: 40 }),
        bullet('Token-efficient web fetch and search tooling for LLM agents, with reference-style URL preservation and total output token budgeting.'),
